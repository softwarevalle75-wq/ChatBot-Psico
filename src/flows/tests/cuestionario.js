...
export const iniciarCuestionario = async (numeroUsuario, msg, tipoTest) => {
	...
	console.log('Iniciando cuestionario para:', numeroUsuario, tipoTest)

	try {
		let estado = await getEstadoCuestionario(numeroUsuario, tipoTest)
		let respuesta = Number(apiCuest(msg, tipoTest))
		if (respuesta === 9) return preguntas[estado.preguntaActual]
		...
		if (!estado.resPreg) {
			estado = {
				...
				preguntaActual: 0,
				resPreg: resPreg,
				respuestasDass21: tipoTest === 'dass21' ? [] : undefined
			}	
		...
		//en caso de estar fuera de rango
		if (![0,1,2,3].includes(respuesta)) {
			return 'Respuesta inválida. Sólo se acepta un número del 0 al 3.'
		}

		// Se actualiza estado con respuesta usuario
		estado.Puntaje += respuesta

		if (!estado.resPreg[respuesta]){
			estado.resPreg[respuesta] = []
		}
		...
		await saveEstadoCuestionario(
			...
			estado.resPreg,
			tipoTest,
			tipoTest === 'dass21' ? estado.respuestasDass21 : undefined
		)
		return preguntas[estado.preguntaActual]

	} catch (error) {
		console.log('error en iniciar cuestionario', error)
		return 'Ocurrió un problema al procesar el cuestionario. Inténtalo de nuevo'
	}	
}
// Evaluar resultado
